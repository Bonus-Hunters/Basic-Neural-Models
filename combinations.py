        ...
        for class_pair in class_combinations:
            ...
            for feature_pair in feature_combinations:
                ...
                try:
                    ...
                    success = save_weights(
                        model, weights_path, feature_names, scaling_params
                    )
                    ...
                    if success:
                        ...
                        print(
                            f"      [FAILED] Could not save weights to: {weights_path}"
                        )
...
if __name__ == "__main__":
    ...
    total_combinations = (
        2
        * len(list(itertools.combinations(classes, 2)))
        * len(list(itertools.combinations(features, 2)))
    )
    print(f"Total possible combinations: {total_combinations}")
    print(
        f"Success rate: {len(results)}/{total_combinations} ({len(results)/total_combinations*100:.1f}%)"
    )
