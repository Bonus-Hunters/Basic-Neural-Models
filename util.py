...
def construct_model_obj(
    ...
    if type.lower() == "perceptron":
        model = Perceptron(learning_rate, max_iterations, use_bias, weights=weights, bias=bias)
    elif type.lower() == "adaline":
        model = Adaline(learning_rate, max_iterations, use_bias, acceptable_error, weights=weights, bias=bias)
    return model
...
def save_model(model, model_name: str, class_pair, feature_pair, savePath="./Models/"):
    ...
    weights_data = {
        "weights": model.weights.tolist() if hasattr(model, 'weights') and model.weights is not None else [],
